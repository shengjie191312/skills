...
def investCalendar(query: str) -> dict:
    """查询投资日历，包括事件日历、催化日历、事件关联投资机会分析。

    不属于该工具的问题包括特定品类的标准化列表查询、单个具体事件的详细查询/进展追踪。

    Args:
        query: 用户问题
    """
    return _post("/api/finAnalysis/investCalendar", {"query": query}, timeout=660)


def diagnosisStock(query: str) -> dict:
    """对个股、基金、ETF、板块等进行分析诊断。

    内部会先做实体识别，若全部为基金则走基金诊断接口，否则回退到 SMA 综合分析。
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    s = sub.add_parser("investCalendar", help="查询投资日历")
...
def main() -> None:
    ...
    if args.tool == "investCalendar":
        result = investCalendar(query=args.query)
    elif args.tool == "diagnosisStock":
