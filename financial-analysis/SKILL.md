...
description: 金融分析与资讯查询。支持投资日历、分析诊断、市场洞察分析。当用户提出金融市场分析、资讯检索、投资日历或分析诊断相关问题时，根据类别路由到对应工具获取专业结果。
user-invocable: true
metadata:
  openclaw:
    emoji: 📊
...
当用户提出金融市场分析、金融资讯查询、投资日历生成或分析诊断相关问题时，根据问题类别路由到对应的工具获取专业结果。
...
- 其他资产查询：基金分析、贵金属分析、期货期权分析、债券分析、其他金融产品
- 金融资讯查询：个股/多股相关新闻资讯检索、利好利空消息分类整理、指定时间范围内的资讯汇总、板块/行业/市场新闻聚合
- 宏观事件与选股/荐股：基于宏观经济、地缘政治、政策变化等事件分析其对金融市场的影响
- 投资日历：公司、行业、板块、全市场事件类查询/汇总/分析，包括事件日历、催化日历
- 分析诊断：个股分析诊断、基金分析诊断、基金持仓分析、基金替换建议
...
### investCalendar
查询投资日历，包括事件日历、催化日历、事件关联投资机会分析。

**何时调用**：用户要求生成围绕公司、行业、板块、全市场事件类查询/汇总/分析。不属于该工具的问题包括特定品类的标准化列表查询、单个具体事件的详细查询/进展追踪。

**参数**：
- `query`：用户问题（必填）

**执行**：`python3 financial_analysis.py investCalendar --query <query>`

---

### diagnosisStock
对个股、基金、ETF、板块等进行分析诊断。内部会先做实体识别，若全部为基金则走基金诊断接口，否则回退到 市场洞察（marketInsight）分析。

**何时调用**：用户要求对个股、基金、ETF、板块等进行分析诊断、诊断报告。
...
| 用户意图 | 类别 | 调用路径 |
|---|---|---|
| "帮我生成比亚迪未来一周事件日历" | 投资日历 | `investCalendar` |
| "未来一周事件速览" | 投资日历 | `investCalendar` |
